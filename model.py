...
class PlackettLuce():
	...
	def __init__(self, rel, len_a):
		...
		self.probs = nn.Softmax(dim=0)(s).data.numpy().flatten()
	...
	def sample_rankings(self, size):
		...
		for i in range(size):
	        ...
	        ranking = ranking.assign({'ranks': ranks})

	        relevances = [self.rel[i] for i in ranks]
	        ranking = ranking.assign({'relevances': relevances})

	        groups = ['A' if i < self.len_a else 'B' for i in ranks]
	        ranking = ranking.assign({'groups': groups})
	...
	def compute_log_probability(self, ranking):
	    ...
	    for j in range(self.s.size()[0]):
	        ...
	        log_probs[j] = self.s[pos_j] - logsumexp(self.s - subtracts, dim=0)
	        subtracts[pos_j] = self.s[pos_j] + 1e6

	    return torch.sum(log_probs)


	def reinforce_loss(rankings):
		"""
		...
		for r in rankings:
			...
			log_prob = compute_log_probability(r)
			loss += float(-(reward - baseline)) * log_prob

		return loss / rankings.shape[0]
