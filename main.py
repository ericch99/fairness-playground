...
def plot_all(k, rank, select, n, big_a, big_b, big_a_1, big_b_1, big_a_2, big_b_2):
    ...
    plt.errorbar(np.arange(n), mean_a, yerr=max_a - min_a, fmt='-', color='C2', label=f'Group A mean (B = -1, A = 0)')
    plt.errorbar(np.arange(n), mean_a_1, yerr=max_a_1 - min_a_1, fmt='--', color='C2', label=f'Group A mean (B = -0.5, A = 0)')
    plt.errorbar(np.arange(n), mean_a_2, yerr=max_a_2 - min_a_2, fmt=':', color='C2', label=f'Group A mean (B = -0.1, A = 0)')
    plt.errorbar(np.arange(n), mean_b, yerr=max_b - min_b, fmt='-', color='C0', label=f'Group B mean (B = -1, A = 0)')
    plt.errorbar(np.arange(n), mean_b_1, yerr=max_b_1 - min_b_1, fmt='--', color='C0', label=f'Group B mean (B = -0.5, A = 0)')
    plt.errorbar(np.arange(n), mean_b_2, yerr=max_b_2 - min_b_2, fmt=':', color='C0', label=f'Group B mean (B = -0.1, A = 0)')
    plt.legend()
    ...
    for k in ks:
        for rank in ranks:
            for select in selects:
                ...
                plot_all(k, rank, select, num_iter, big_a, big_b, big_a_1, big_b_1, big_a_2, big_b_2)
